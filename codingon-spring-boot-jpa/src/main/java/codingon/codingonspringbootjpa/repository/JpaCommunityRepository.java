...
public interface JpaCommunityRepository extends JpaRepository<JpaCommunityEntity, Integer> {
    ...
    List<JpaCommunityEntity> findByTitleOrContentOrWriter(String title, String content, String Writer);
    
    // 반환 형식은 Entity, List<Entity> 모두 가능
    // 단, 반환 된 값이 1개를 넘는데 Entity 로 넣으려고 하면 문제 발생

    Optional<JpaCommunityEntity> findById(int id);
    // Query 어노테이션
